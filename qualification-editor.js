// Safe Site - Qualification Editor Update
// Pilot feature: tap an existing qualification to edit its name, issue date, or expiry date.
(function () {
  ...
  function resetEditorTitle() {
    ...
    const save = screen && [...screen.querySelectorAll('button')]
      .find(b => /save qualification|save changes/i.test(b.textContent));
  ...
  function makeQualificationRowsEditable() {
    ...
    rows.forEach((row, index) => {
      ...
      const textArea = row.querySelector('.grow') || row;
      textArea.appendChild(hint);

      row.addEventListener('click', () => openQualificationEditor(q));
      row.addEventListener('keydown', e => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault();
          openQualificationEditor(q);
        }
      });
    });
  }
  ...
  if (typeof originalShow === 'function') {
    window.show = function (screen) {
      if (screen === 'qualificationEditor' && !editingQualificationId) {
        resetEditorTitle();
      }
  ...
  if (typeof originalSaveQualification === 'function') {
    window.saveQualification = async function () {
      if (!editingQualificationId) {
        return originalSaveQualification.apply(this, arguments);
      }
      ...
      const payload = {
        name,
        issued_on: getEl('qualIssued')?.value || null,
        expires_on: getEl('qualExpires')?.value || null
      };
      ...
      if (error) {
        console.error(error);
        toast('Could not update qualification');
        return;
      }

      if (typeof logAudit === 'function') {
        logAudit('updated', 'qualification', `${worker.name}: ${name}`);
      }
  ...
  const list = getEl('qualList');
  if (list) {
    new MutationObserver(makeQualificationRowsEditable)
