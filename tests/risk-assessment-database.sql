-- Run as postgres in the Supabase SQL editor after database/risk_assessment_approval.sql.
...
begin
  ...
  values(pg_temp.f('org'),pg_temp.f('site'),auth.uid(),'pre_task_risk_assessment','Test task','Test task',area,s,
    jsonb_build_object('crew','Test crew','area',area,'hazards','Falling material','controls','Barricade','taskTemplateId',pg_temp.f('task'),'initialLikelihood',3,'initialSeverity',4,'residualLikelihood',2,'residualSeverity',3)||extra)
...
insert into auth.users(id) select id from fixture where key in
  ('worker','other_worker','supervisor','administrator','safety_coordinator','client_viewer','outsider','inactive');
insert into public.organizations(id,name) values(pg_temp.f('org'),'Risk-assessment test'),(pg_temp.f('other_org'),'Other test org');
...
begin
  ...
  perform pg_temp.blocked(format('insert into public.safety_records(organization_id,site_id,created_by,record_type,work_area,status,approved_by) values(%L,%L,%L,''pre_task_risk_assessment'',''Area'',''pending_review'',%L)',pg_temp.f('org'),pg_temp.f('site'),auth.uid(),pg_temp.f('supervisor')),'top-level approver spoof denied');
  perform pg_temp.blocked(format('insert into public.safety_records(organization_id,site_id,created_by,record_type,work_area,status) values(%L,%L,%L,''pre_task_risk_assessment'',''Area'',''pending_review'')',pg_temp.f('org'),pg_temp.f('foreign_site'),auth.uid()),'cross-organization site denied');
...
select set_config('request.jwt.claim.sub',pg_temp.f('worker')::text,true);
select pg_temp.blocked($q$select pg_temp.submit(extra=>'{"residualLikelihood":5,"residualSeverity":5,"residualScore":1}')$q$,'forged low score cannot bypass stop-work threshold');
select pg_temp.blocked($q$select pg_temp.submit(extra=>'{"residualLikelihood":2,"residualSeverity":5}')$q$,'exact stop-work boundary denied');
select pg_temp.blocked($q$select pg_temp.submit(extra=>'{"residualLikelihood":1.5}')$q$,'fractional ratings denied');
select pg_temp.blocked($q$select pg_temp.submit(extra=>'{"residualSeverity":0}')$q$,'out-of-range ratings denied');
select pg_temp.blocked($q$select pg_temp.submit(extra=>'{"initialLikelihood":"3"}')$q$,'string ratings denied');
select pg_temp.blocked($q$select pg_temp.submit(extra=>'{"residualSeverity":null}')$q$,'null ratings denied');
select pg_temp.blocked($q$select pg_temp.submit(extra=>'{"aiAudit":{"supervisorReviewCompletedAt":"2026-01-01"}}')$q$,'AI draft cannot claim supervisor review');
select pg_temp.blocked($q$select pg_temp.submit(extra=>'{"stopWorkThresholdAtSubmission":25}')$q$,'client cannot choose server threshold');
do $$
declare rec uuid;
begin
  rec:=pg_temp.submit(extra=>'{"initialScore":1,"residualScore":1,"residualLevel":"Low"}');
  perform pg_temp.ok((select data->>'residualScore'='6' and data->>'initialScore'='12' and data->>'residualLevel'='Medium'
    and data->>'stopWorkThresholdAtSubmission'='10' from public.safety_records where id=rec),'server recomputes scores and threshold');
end $$;
select set_config('test.threshold_record',pg_temp.submit()::text,true);
select set_config('request.jwt.claim.sub',pg_temp.f('administrator')::text,true);
update public.sites set risk_stop_work_threshold=6 where id=pg_temp.f('site');
select pg_temp.blocked(format('update public.safety_records set status=''approved'' where id=%L',current_setting('test.threshold_record')),'approval rechecks current site threshold');
select set_config('request.jwt.claim.sub',pg_temp.f('worker')::text,true);
select pg_temp.blocked('select pg_temp.submit()','submission enforces changed site threshold');
select pg_temp.no_rows(format('update public.sites set risk_stop_work_threshold=25 where id=%L',pg_temp.f('site')),'worker cannot relax stop-work threshold');
reset role;
