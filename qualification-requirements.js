...
(function () {
  ...
  async function loadJurisdictions() {
    ...
    if (!select || typeof initSupabase !== 'function') return;
    const client = initSupabase();
    const { data, error } = await client.from('jurisdictions').select('code,name').eq('country_code',country).eq('active',true).order('name');
    if (error) return console.warn(error);
    select.innerHTML = (data || []).map(x => '<option value="' + x.code + '">' + x.name + '</option>').join('');
    document.getElementById('qrAdminRegulator').value = country === 'US' ? 'MSHA' : 'Provincial/Territorial';
  }
